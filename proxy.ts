...
export async function proxy(request: NextRequest) {
  ...
  if (!user && ["/dashboard", "/profile", "/resume", "/career", "/roadmap", "/history"].some((path) => request.nextUrl.pathname.startsWith(path))) { const url = request.nextUrl.clone(); url.pathname = "/login"; url.searchParams.set("next", request.nextUrl.pathname); return NextResponse.redirect(url) }
  return response
}
